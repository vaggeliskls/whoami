\documentclass[a4paper,10pt]{article}
\usepackage[left=1.5cm, right=1.5cm, top=1.5cm, bottom=1.5cm]{geometry}
\usepackage{xcolor, titlesec, enumitem, fontawesome5, hyperref, parskip}
\usepackage{graphicx}

% Define Colors
\definecolor{primary}{RGB}{41, 128, 185}
\definecolor{secondary}{RGB}{44, 62, 80}

% Title Formatting
\titleformat{\section}{\large\bfseries\color{primary}}{}{0em}{}[\titlerule]
\titleformat{\subsection}{\bfseries\color{secondary}}{}{0em}{}
% Long table definition
\usepackage{longtable}
\renewcommand{\arraystretch}{1.3} % Adjust row spacing


\begin{document}
\pagestyle{empty}

% ----------------------- Header section -----------------------
\begin{center}
    {\huge \textbf{Vaggelis Kalligiannakis} \faIcon{laptop-code}} \\
    \vspace{7pt}
    {\large DevOps Cloud Engineer \& Full-Stack Developer} \\
    \vspace{7pt}
    \faIcon{map-marker-alt} Heraklion Crete, GR (UTC+2) \quad
    \faIcon{envelope} \href{mailto:vaggeliskls@gmail.com}{vaggeliskls@gmail.com} \quad
    \faIcon{github} \href{https://github.com/vaggeliskls}{github.com/vaggeliskls} \quad
    \faIcon{dev} \href{https://dev.to/vaggeliskls}{dev.to/vaggeliskls} \quad
    \faIcon{linkedin} \href{https://www.linkedin.com/in/vaggelis-kls-5817bb47/}{linkedin.com/in/vaggelis-kls-5817bb47} \quad
    \faIcon{medium} \href{https://medium.com/@vaggelis.vk}{medium.com/@vaggelis.vk}
\end{center}

\vspace{4pt}

% ----------------------- About me -----------------------
\section*{\faIcon{user} About Me}
Experienced \textbf{Cloud DevOps Engineer} and \textbf{Full-Stack Software Developer} with expertise in \textbf{cloud infrastructure, automation, and scalable architectures}.  
Passionate about \textbf{open-source development, research, and technical writing}.

\vspace{2pt}

% ----------------------- Skills -----------------------
\section*{\faIcon{tools} Technical Skills}
\begin{itemize}[left=0pt, label=\faIcon{server}]
    \item \textbf{Cloud \& DevOps:} Kubernetes, Docker, OpenShift, AWS, GCP, Terraform, Helm, ArgoCD
\end{itemize}
\begin{itemize}[left=0pt, label=\faIcon{code}]
    \item \textbf{Programming:} Python, TypeScript, Go, Node.js, FastAPI, React, Next.js
\end{itemize}
\begin{itemize}[left=0pt, label=\faIcon{lock}]
    \item \textbf{Infrastructure \& Security:} Linux, Bash, CI/CD, OAuth, LDAP, SSL/TLS
\end{itemize}
\begin{itemize}[left=0pt, label=\faIcon{chart-line}]
    \item \textbf{Monitoring \& Observability:} Prometheus, Grafana, ELK Stack
\end{itemize}
\begin{itemize}[left=0pt, label=\faIcon{brain}]
    \item \textbf{Machine Learning:} MLOps, Model Deployment, Data Pipelines
\end{itemize}

\vspace{2pt}

% ----------------------- Work Experience -----------------------
% \section*{\faIcon{briefcase} Work Experience}

% \begin{longtable}{r|p{12.5cm}}

% % TurinTech AI
% \textsc{Jul 2021 – Present} & \textbf{Lead Cloud DevOps Engineer \& Full-Stack Software Engineer} \\
% \footnotesize{\textit{London, UK (Remote)}} & \textsc{TurinTech AI} \\
% & Architected and deployed \textbf{Kubernetes-based infrastructure}, improving scalability. \\
% & Led the development of \textbf{CI/CD pipelines} (GitHub Actions, ArgoCD), reducing deployment time by \textbf{50\%}. \\
% & Designed and enforced \textbf{cloud security best practices} across multiple projects. \\
% & Integrated \textbf{MLOps workflows} to streamline machine learning deployment. \\

% % FORTH - Software Engineer
% \textsc{Sep 2017 – Aug 2021} & \textbf{Software Engineer} \\
% \footnotesize{\textit{Heraklion, Greece}} & \textsc{Foundation for Research and Technology - Hellas (FORTH)} \\
% & Researched and developed \textbf{Ambient Intelligence} solutions for smart environments. \\
% & Created \textbf{cloud-based data pipelines} for real-time sensor data processing. \\
% & Published research on \textbf{AI, IoT, and distributed computing}. \\

% % FORTH - Research & Development Engineer
% \textsc{Nov 2014 – Jan 2016} & \textbf{Research \& Development Engineer} \\
% \footnotesize{\textit{Heraklion, Greece}} & \textsc{Foundation for Research and Technology - Hellas (FORTH)} \\
% & Developed \textbf{distributed systems} for \textbf{big data analytics} and \textbf{semantic web applications}. \\

% \end{longtable}


\section*{\faIcon{briefcase} Professional Experience}

\subsection*{TurinTech AI \textbar\ Lead Cloud DevOps Engineer \textbar\ Jul 2021 - Present (3 yrs 8 mo)}
\begin{itemize}[left=0pt]
    \item Architected and deployed \textbf{Kubernetes-based infrastructure}, improving scalability.
    \item Designed \textbf{CI/CD pipelines} (GitHub Actions, ArgoCD), reducing deployment time by \textbf{50\%}.
    \item Led a team of engineers in \textbf{automating cloud infrastructure} and enforcing security best practices.
    \item Developed \textbf{MLOps solutions}, integrating machine learning pipelines.
\end{itemize}

\subsection*{Foundation for Research and Technology - Hellas (HCI) \textbar\ Software Engineer \textbar\ Sep 2017 - Aug 2021 (4 yrs)}
\begin{itemize}[left=0pt]
    \item Researched and developed \textbf{Ambient Intelligence} solutions for smart environments.
    \item Created \textbf{cloud-based data pipelines} for processing real-time sensor data.
    \item Published research on \textbf{AI, IoT, and distributed computing}.
\end{itemize}

\subsection*{Foundation for Research and Technology - GR (ISL) \textbar\ Research \& Development Engineer \textbar\ Nov 2014 - Jan 2016 (1 yr 3 m)}
\begin{itemize}[left=0pt]
    \item Developed distributed systems for \textbf{big data analytics} and \textbf{semantic web applications}.
\end{itemize}

\subsection*{Foundation for Research and Technology - GR (ISL) \textbar\ Research \& Development Engineer \textbar\ Jun 2013 - Aug 2013 (3 m)}
\begin{itemize}[left=0pt]
    \item Developed distributed systems for \textbf{big data analytics} and \textbf{semantic web applications}.
\end{itemize}


\vspace{2pt}

% ----------------------- Education -----------------------
\section*{\faIcon{graduation-cap} Education}

\subsection*{University of Crete \textbar\ Master's Degree in Computer Science \textbar\ 2014 - 2016}
\begin{itemize}[left=0pt]
    \item Focus: \textbf{Semantic Web, Big Data, IoT, Distributed Systems}
    \item \textbf{Postgraduate Scholarship} for highest GPA achievement
\end{itemize}

\subsection*{University of the Aegean \textbar\ Bachelor's Degree in Information \& Communication Systems Engineering \textbar\ 2007 - 2013}
\begin{itemize}[left=0pt]
    \item \textbf{GeoSpatial Web Services project} (Microsoft Imagine Cup finalist)
\end{itemize}

\vspace{2pt}

% ----------------------- Publications -----------------------
\section*{\faIcon{book} Publications}
\subsection*{Integrating Ambient Intelligence Technologies for Empowering Agriculture}
\textit{EFITA Conference, 2021}  

\subsection*{Ambient Intelligence in the Living Room}
\textit{Sensors Journal, 2019}  

\subsection*{AmITV: Enhancing the Role of TV in Ambient Intelligence Environments}
\textit{ACM PETRA Conference, 2018}  

\vspace{2pt}

\end{document}
